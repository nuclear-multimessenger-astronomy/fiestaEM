...
for metric_name in ["L2", "Linf"]:
    ...
    benchmarker = Benchmarker(
                    model = lc_model,
                    data_file = "../data/Bu2024_raw_data.h5",
                    metric_name = metric_name
                    )
    
    benchmarker.benchmark()

    benchmarker.plot_lightcurves_mismatch(parameter_labels = ["$\\log_{10}(m_{\\mathrm{ej, dyn}})$", "$v_{\\mathrm{ej, dyn}}$", "$Y_{e, \\mathrm{dyn}}$", "$\\log_{10}(m_{\\mathrm{ej, wind}})$", "$v_{\\mathrm{ej, wind}}$", "$\\iota$", "$z$"])
