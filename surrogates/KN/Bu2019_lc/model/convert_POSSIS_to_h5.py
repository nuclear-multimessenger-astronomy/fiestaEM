...
from fiesta.conversions import Flambda_to_Fnu, Fnu_to_Flambda
c = 299792458


#################################################################################
dir = "../../../flux_models/Bu2019/model/2020_dietrich_Bu2019lm"
...
####################
# get the files
files = [f for f in os.listdir(dir) if ".txt" in f]
...
for f in tqdm.tqdm(files):
    ...
    for j, obs in enumerate(obs_angles):
        ...
        params.append(obs)
        X.append(params)
        Flambda = a[j * n_nus : (j+1) * n_nus, 1:]
        Fnu, _ = Flambda_to_Fnu(Flambda, lambdas)
        y.append(Fnu)
