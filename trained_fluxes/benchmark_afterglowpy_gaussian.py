...
for metric_name in ["$\\mathcal{L}_2$", "$\\mathcal{L}_\infty$"]:   
    ...
    for filt in FILTERS:
        ...
        fig, ax = B.plot_lightcurves_mismatch(filter =filt, parameter_labels = ["$\\iota$", "$\log_{10}(E_0)$", "$\\alpha_c$", "$\\theta_{\\mathrm{w}}$", "$\log_{10}(n_{\mathrm{ism}})$", "$p$", "$\\epsilon_E$", "$\\epsilon_B$"])
...
fig.savefig(f"./benchmarks/{name}/error_distribution.pdf", dpi = 200)

fig, ax = B.plot_error_over_time()
fig.savefig(f"./benchmarks/{name}/error_over_time.pdf", dpi = 200)
